...
We will use 2 two learning algorithms to compare accuracy.
...
Out of sample error is `r (1 - (sum(prediction_rpart == testing$classe)/length(prediction_rpart)))`.
...
Clearly rf method has better accuracy. We will continue to use Random Forest (rf) as the learning algorithm to obtain better predictive performance and find best predictors for classe variable.
